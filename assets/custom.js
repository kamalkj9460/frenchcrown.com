...
function initFunc(){
  ...
  $('body').on('click',function(){
    $('.modal.modal-size').removeClass('show');
    ...
    $('.modal-size .modal-inner').on('click',function(event){
      event.stopPropagation();
    });
  })
...
$(document).on('click', '[name=checkout]', function(event) {
  ...
  $.getJSON('/cart.js', function(cart) {
    if(cart.items.length == 1 && cart.items[0].product_id == 6957872251020){
...
/* set loader on checkout button click end */
